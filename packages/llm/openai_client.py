...
from utils.secrets_manager import Safebox

from llm.base import LLMClientBase
from llm.types import JobInfo, JobInfoType, Prompt
from llm.utils import OPENAI_COST, LLMEngine
...
class OpenAIClient(LLMClientBase):
    def __init__(self, engine: LLMEngine, prompt: Prompt):
        self.logger = initiate_logger(self.__class__.__name__)
        self.client = OpenAI(secrets.openai_api_key)
        self.engine = engine
        self.prompt = prompt
        self.job_info = JobInfoType(individual=[], aggregated={})

    def call_request(self, job_name: str) -> Response:
        if job_name == "chat_completion":
            self.logger.info(f"Calling OpenAI API and request with {job_name} job...")
            response = self.__chat_completion()
        else:
            self.logger.error(f"The {job_name} job is not currently available!")
